...
@section('content')
            ...
            <div class="card mb-12">
                <div class="card-header">
                    @if (auth::user()->role != 'taruna')
                        <a href="#" class="btn btn-sm btn-primary" data-toggle="modal" data-target=".modalform"
                            data-array=""><i class="fas fa-plus"></i> Tambah</a>

                        <a href="{{ url('nilaisamaptaexport') }}/{{ @$_GET['id_semester'] }}"
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped" width="100%" id="dataTable" cellspacing="0"
                            ...
                            <thead>
                                <tr>
                                    <th style="width: 20px; vertical-align: middle; text-align: center;" rowspan="2">No
                                    </th>
                                    <th style="width: 100px; vertical-align: middle; text-align: center;" rowspan="2">
                                        Taruna</th>
                                        ...
                                        @if (auth::user()->role != 'taruna')
                                            <td>
                                                <a href="#" data-toggle="modal" data-target=".modalform"
                                                    data-array="{{ json_encode($data[$k]) }}"
                                                    class="btn btn-sm btn-success btn-block"><i class="fas fa-edit"></i></a>
                                                <a class="btn btn-sm btn-primary btn-block mt-1"
                                                    href="{{ url('nilaisamaptaexportpdf') }}/{{ $item->id_nilai_samapta }}">
                                                ...
                                                <div class="modal fade" id="hapus{{ $item->id_nilai_samapta }}"
    ...
    <div class="modal fade modalform" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form class="form" method="post">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            ?>
                            <input readonly
                                value="{{ Semester::where('is_semester_aktif', '1')->value('nama_semester') }}"
                                class="form-control">
                        </div>
                        <?php $aturan = DB::table('aturan_nilai_samaptas')->get(); ?>
                        <div class="form-group">
                            <label for="">Jarak Lari (meter)</label>
                            <input type="number" class="form-control" id="lari" name="lari" required
                                placeholder="isi dengan angka tanpa koma">
                            <div class="mt-2 mb-4" style="font-size: 12px;">
                                <p><i class="fas fa-info-circle"></i>&nbsp;
                                    max jarak lari taruna:
                                    {{ $aturan->where('untuk', 'Taruna')->where('jenis_samapta', 'Lari')->max('jumlah') }},
                                    max jarak lari taruni:
                                    {{ $aturan->where('untuk', 'Taruni')->where('jenis_samapta', 'Lari')->max('jumlah') }}
                                    <br>
                                    <i class="fas fa-info-circle"></i>&nbsp;
                                    min jarak lari taruna:
                                    {{ $aturan->where('untuk', 'Taruna')->where('jenis_samapta', 'Lari')->min('jumlah') }},
                                    min jarak lari taruni:
                                    {{ $aturan->where('untuk', 'Taruni')->where('jenis_samapta', 'Lari')->min('jumlah') }}
                                </p>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="">Push Up (JML)</label>
                            <input type="number" class="form-control" id="pushup" name="pushup" required
                                placeholder="isi dengan angka tanpa koma">
                            <div class="mt-2 mb-4" style="font-size: 12px;">
                                <p><i class="fas fa-info-circle"></i>&nbsp;
                                    max push up taruna:
                                    {{ $aturan->where('untuk', 'Taruna')->where('jenis_samapta', 'Push-up')->max('jumlah') }},
                                    max push up taruni:
                                    {{ $aturan->where('untuk', 'Taruni')->where('jenis_samapta', 'Push-up')->max('jumlah') }}
                                    <br>
                                    <i class="fas fa-info-circle"></i>&nbsp;
                                    min push up taruna:
                                    {{ $aturan->where('untuk', 'Taruna')->where('jenis_samapta', 'Push-up')->min('jumlah') }},
                                    min push up taruni:
                                    {{ $aturan->where('untuk', 'Taruni')->where('jenis_samapta', 'Push-up')->min('jumlah') }}
                                </p>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="">Sit Up (JML)</label>
                            <input type="number" class="form-control" id="situp" name="situp" required
                                placeholder="isi dengan angka tanpa koma">
                            <div class="mt-2 mb-4" style="font-size: 12px;">
                                <p><i class="fas fa-info-circle"></i>&nbsp;
                                    max sit up taruna:
                                    {{ $aturan->where('untuk', 'Taruna')->where('jenis_samapta', 'Sit-up')->max('jumlah') }},
                                    max sit up taruni:
                                    {{ $aturan->where('untuk', 'Taruni')->where('jenis_samapta', 'Sit-up')->max('jumlah') }}
                                    <br>
                                    <i class="fas fa-info-circle"></i>&nbsp;
                                    min sit up taruna:
                                    {{ $aturan->where('untuk', 'Taruna')->where('jenis_samapta', 'Sit-up')->min('jumlah') }},
                                    min sit up taruni:
                                    {{ $aturan->where('untuk', 'Taruni')->where('jenis_samapta', 'Sit-up')->min('jumlah') }}
                                </p>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="mt-2 mb-4" style="font-size: 12px;">
                                <p><i class="fas fa-info-circle"></i>&nbsp;
                                    max shuttle run taruna:
                                    {{ $aturan->where('untuk', 'Taruna')->where('jenis_samapta', 'Shuttle Run')->max('jumlah') }},
                                    max shuttle run taruni:
                                    {{ $aturan->where('untuk', 'Taruni')->where('jenis_samapta', 'Shuttle Run')->max('jumlah') }}
                                    <br>
                                    <i class="fas fa-info-circle"></i>&nbsp;
                                    min shuttle run taruna:
                                    {{ $aturan->where('untuk', 'Taruna')->where('jenis_samapta', 'Shuttle Run')->min('jumlah') }},
                                    min shuttle run taruni:
                                    {{ $aturan->where('untuk', 'Taruni')->where('jenis_samapta', 'Shuttle Run')->min('jumlah') }}
                                </p>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" class="form-control" id="tinggibadan"
                                name="tinggibadan" required placeholder="isi dengan angka, gunakan titik untuk koma">
                        </div>
                        <div class="form-group">
